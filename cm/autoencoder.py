...
class QKVFlashAttention(nn.Module):
    ...
    def forward(self, qkv, attn_mask=None, key_padding_mask=None, need_weights=False):
        ...
        qkv, _ = self.inner_attn(
            qkv.contiguous(),  # https://github.com/openai/consistency_models/issues/3
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.input_blocks = nn.ModuleList(
            [TimestepEmbedSequential(conv_nd(dims, ch, ch, 3, padding=1))]
        )
        ds = 2 ** len(channel_mult)
...
class Autoencoder(nn.Module):
    ...
    def encode(self, x, timesteps, y=None):
        return self.encoder(x, timesteps, y)
    
    def decode(self, x, timesteps, y=None):
        return self.decoder(x, timesteps, y)

    def forward(self, x, input_timesteps, output_timesteps, y=None):
        """
        Apply the model to an input batch.

        :param x: an [N x C x ...] Tensor of inputs.
        :param input_timesteps: a 1-D batch of timesteps.
        :param output_timesteps: a 1-D batch of timesteps.
        :param y: an [N] Tensor of labels, if class-conditional.
        :return: an [N x C x ...] Tensor of outputs.
        """
        return self.decode(self.encode(x, input_timesteps, y), output_timesteps, y)
...
#     print(model.encoder)
#     print(model.decoder)
#     x = th.randn(1, 3, 64, 64, device=device)
#     input_timesteps = th.tensor([0], device=device)
#     output_timesteps = th.tensor([1], device=device)
#     print(model(x, input_timesteps, output_timesteps).shape)
